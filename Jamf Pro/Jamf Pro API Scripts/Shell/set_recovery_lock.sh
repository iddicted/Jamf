...
JamfProURL="JAMF_PRO_URL_HERE" # Replace with your Jamf Pro URL (e.g., "https://example.jamfcloud.com:")
...
get_group_info() {
	curl --silent --request GET \
		--url "$JamfProURL/api/v2/computer-groups/smart-groups?page=0&page-size=100&sort=id%3Aasc&filter=name==%22${group_name// /%20}%22" \
...
if [[ $(echo "$group_exists" | jq -r '.results | length') -gt 0 ]]; then
	...
	create_response=$(curl --silent --request POST \
		--url "$JamfProURL/api/v2/computer-groups/smart-groups" \
		--header "Authorization: Bearer $access_token" \
		--header 'accept: application/json' \
		--header 'content-type: application/json' \
		--data "{
...
group_members=$(curl --request GET \
	--url "$JamfProURL/api/v2/computer-groups/smart-group-membership/$group_id" \
	--silent \
	--header 'accept: application/json' \
	--header "Authorization: Bearer ${access_token}")
